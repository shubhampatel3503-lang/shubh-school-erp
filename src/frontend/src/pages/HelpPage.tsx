...
const MODULES: ModuleDoc[] = [
  {
    ...
    steps: [
      ...
      },
      {
        ...
        detail:
          "See total students, fee collection today (₹), attendance %, and pending tasks",
      },
      {
        step: "Search for a student",
        detail:
          "Type a name, mobile number, village, or parent name in the search bar",
      },
      {
      ...
      },
      {
        step: "Change session",
        detail:
          "Use the session selector (top bar) to switch between academic years like 2024–25 or 2025–26",
      },
    ],
    features: [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        step: "Add a class",
        detail:
          "Click 'Add Class', enter class name (e.g. Class 5), section, and capacity",
      },
      {
        step: "Add subjects",
        detail:
          "Select a class, then click 'Add Subject'. Enter subject name, code, and max marks",
      },
      {
      ...
      },
      {
        ...
        detail:
          "For each subject, click 'Syllabus' to upload a PDF or enter chapter-wise topics",
      },
    ],
    features: [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        detail:
          "Click 'Add Student', fill the form with name, father's name, class, section, mobile, DOB (dd/mm/yyyy), and address",
      },
      {
        step: "Upload student photo",
        detail:
          "Click the photo area → select a JPG/PNG file from your device (no URL needed)",
      },
      {
        step: "Assign discount",
        detail:
          "Open student profile → Fee tab → click 'Discount' to set monthly discounts per fee head",
      },
      {
        step: "Set old balance",
        detail:
          "In the Fee tab, enter previous session outstanding balance — it auto-adds to current dues",
      },
      {
        step: "Export student list",
        detail:
          "Click Export → check the columns you want to share → Download CSV",
      },
      {
        step: "Bulk import",
        detail:
          "Click Import → download the sample format → fill data → upload the file",
      },
    ],
    features: [
    ...
    steps: [
      {
        ...
        detail:
          "Fees → Fee Headings → Add Heading (e.g. Tuition Fee, Transport, Meal)",
      },
      {
        step: "Create a Fee Plan",
        detail:
          "Fees → Fee Plans → Add Plan → select class → enter monthly amounts for each heading",
      },
      {
        step: "Collect Fees",
        detail:
          "Fees → Collect Fees → search student → review dues with discounts applied → collect amount → generate receipt",
      },
      {
        step: "View Fee Register",
        detail:
          "Fees → Fee Register → filter by date, class, or user to see all collections",
      },
      {
        step: "User-wise report",
        detail:
          "Admin can filter Fee Register by user to see which staff collected how much and for which students",
      },
    ],
    features: [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        detail:
          "Click P (Present), A (Absent), L (Late), or H (Half Day) for each student",
      },
      {
      ...
      },
      {
        step: "View summary",
        detail:
          "Click 'Attendance Summary' to see monthly attendance % per student",
      },
      {
        step: "Export",
    ...
    steps: [
      {
        step: "Create an Exam",
        detail:
          "Examinations → Add Exam → enter name (e.g. Half Yearly), start/end dates, exam type",
      },
      {
        step: "Generate Timetable",
        detail:
          "Click 'Generate Timetable' → AI suggests subject schedule → drag and drop to adjust → save",
      },
      {
        step: "Enter Marks",
        detail:
          "Select exam → select class → enter marks for each student per subject",
      },
      {
        step: "Generate Results",
        detail:
          "Click 'Generate Results' to calculate total, percentage, and rank",
      },
      {
      ...
      },
      {
        ...
        detail:
          "Online Exams tab → Add Questions → set duration → assign to class → students take it online",
      },
    ],
    features: [
    ...
    steps: [
      {
        step: "Add Staff",
        detail:
          "HR → Staff → Add Staff → fill name, designation, department, joining date, mobile, salary",
      },
      {
        step: "Set Salary Structure",
        detail:
          "Click on a staff member → Salary tab → set basic pay + allowances + deductions",
      },
      {
        step: "Process Payroll",
        detail:
          "HR → Payroll → select month → click 'Process All' to calculate net pay for all staff",
      },
      {
        step: "Mark Leave",
    ...
    steps: [
      {
        step: "Add a Vehicle",
        detail:
          "Transport → Vehicles → Add Vehicle → enter bus number, capacity, driver name",
      },
      {
        step: "Create a Route",
        detail:
          "Transport → Routes → Add Route → enter route name and list of stops with distances",
      },
      {
        step: "Assign Students",
        detail:
          "Select a route → Assign Students → pick students from the list",
      },
      {
        step: "Set Transport Fee",
        detail:
          "Go to Fees → Fee Headings → add 'Transport Fee' → include in class fee plan",
      },
      {
        step: "View Route Report",
        detail:
          "Transport → Reports → see how many students per route, driver details, vehicle capacity",
      },
    ],
    features: [
    ...
    steps: [
      {
        step: "Add Books",
        detail:
          "Library → Books → Add Book → enter title, author, publisher, ISBN, quantity, shelf number",
      },
      {
        step: "Issue a Book",
        detail:
          "Library → Issue → search student/staff → select book → set return date → Issue",
      },
      {
        step: "Return a Book",
        detail:
          "Library → Returns → search by student or book → click Return → fine auto-calculated if overdue",
      },
      {
        step: "View Report",
        detail:
          "Library → Reports → see issued books list, overdue list, most borrowed books",
      },
    ],
    features: [
    ...
    steps: [
      {
        ...
        detail:
          "Inventory → Categories → Add (e.g. Furniture, Stationery, Lab Equipment)",
      },
      {
        step: "Add Item",
        detail:
          "Inventory → Items → Add Item → enter name, category, unit, current stock, minimum stock",
      },
      {
        step: "Record Purchase",
        detail:
          "Inventory → Purchase → Add Purchase → select item, quantity, price per unit, date, vendor",
      },
      {
        step: "Issue to Department",
        detail:
          "Inventory → Issue → select item and department → enter quantity → Issue",
      },
      {
        step: "Check Stock",
        detail:
          "Inventory → Stock Report → see current levels, items below minimum stock (highlighted in red)",
      },
    ],
    features: [
    ...
    steps: [
      {
        step: "Send a Notice",
        detail:
          "Communication → Notices → Add Notice → enter title, message, select recipients (All/Class/Group) → Send",
      },
      {
        step: "Send SMS",
        detail:
          "Communication → SMS → compose message → select recipients → Send (requires SMS gateway config)",
      },
      {
        step: "Schedule an Announcement",
        detail:
          "Set a future date while creating a notice — it will auto-publish on that date",
      },
      {
        step: "View Notice Board",
        detail:
          "Communication → Notice Board → shows all active notices sorted by date",
      },
    ],
    features: [
    ...
    steps: [
      {
        ...
        detail:
          "Virtual Classes → Add Class → enter subject, class, teacher, date, time, duration",
      },
      {
      ...
      },
      {
        ...
        detail:
          "Click 'Notify' to send the link to all students in the selected class via notice board",
      },
      {
        step: "View History",
        detail:
          "Virtual Classes → History → see all past sessions with attendance records",
      },
    ],
    features: [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        detail:
          "Click 'New Chat' → search for a staff member or group → type and send",
      },
      {
        step: "Create a group",
    ...
    steps: [
      {
        ...
        detail:
          "Expenses → Categories → Add (e.g. Salary, Electricity, Maintenance, Purchase)",
      },
      {
        step: "Record Expense",
        detail:
          "Expenses → Add Expense → enter amount, date (dd/mm/yyyy), category, paid to, note",
      },
      {
        step: "Attach Voucher",
      ...
      },
      {
        step: "View Summary",
        detail:
          "Expenses → Reports → filter by month or category to see total spending",
      },
    ],
    features: [
    ...
    steps: [
      {
        ...
        detail:
          "Homework → Add Homework → select class, section, subject, date (dd/mm/yyyy), description",
      },
      {
        step: "Set Due Date",
        detail:
          "Enter the due date — students see this on their dashboard with a countdown",
      },
      {
      ...
      },
      {
        step: "Add Remark",
        detail:
          "Click on a student's name → add a remark like 'Excellent work' or 'Incomplete'",
      },
    ],
    features: [
    ...
    steps: [
      {
        ...
        detail:
          "Certificate Studio → choose from Bonafide, TC, ID Card, Admit Card, Fee Receipt",
      },
      {
        step: "Pick a Template",
        detail:
          "Each document has 2–3 pre-designed templates. Click a template to load it",
      },
      {
        step: "Drag Fields to Position",
        detail:
          "Drag text or data fields (student name, class, date, etc.) anywhere on the canvas",
      },
      {
        step: "Save Template",
        detail:
          "Click 'Save Template' — positions are stored permanently in the database",
      },
      {
        step: "Print Document",
        detail:
          "Select a student → click Print → the document fills with real student data → Print or Save PDF",
      },
    ],
    features: [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        detail:
          "Use dropdowns to narrow the view to a specific class or session",
      },
      {
        step: "Export chart",
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        detail:
          "Choose from Student Register, Fee Ledger, Attendance Report, Result Report, etc.",
      },
      {
        step: "Set filters",
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        step: "Set as active",
        detail:
          "Click the 'Set Active' button next to the session you want to work in",
      },
      {
        step: "Switch session",
        detail:
          "Use the session selector on the Dashboard top bar to view historical data",
      },
    ],
    features: [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        step: "Add a user",
        detail:
          "Click 'Add User' → enter full name, username, password, and role (Admin/Teacher/Accountant/Librarian)",
      },
      {
        step: "Set permissions",
        detail:
          "Choose which modules the user can access (view only or full access)",
      },
      {
        step: "Reset password",
        detail:
          "Click on a user → Reset Password → set a new temporary password",
      },
      {
        step: "Deactivate a user",
        detail:
          "Toggle the 'Active' switch to disable login without deleting the account",
      },
    ],
    features: [
    ...
    steps: [
      {
        ...
        detail:
          "Settings → School Profile → enter school name, address, phone, affiliation number, logo upload",
      },
      {
        step: "Change Theme",
        detail:
          "Settings → Theme → select one of 10 colour themes → Apply (takes effect immediately)",
      },
      {
        step: "Edit Landing Page",
        detail:
          "Settings → Landing Page → customise hero text, images, section order, visibility of each section, and colours",
      },
      {
        step: "Configure SMS",
        detail:
          "Settings → Integrations → SMS → enter your gateway API key and sender ID",
      },
      {
        step: "Set up Payment Gateway",
        detail:
          "Settings → Integrations → Payments → add Razorpay/PayU key and secret",
      },
    ],
    features: [
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        step: "Add Alumni",
        detail:
          "Click 'Add Alumni' → enter name, passing year, class passed, current profession, mobile",
      },
      {
        step: "Import from Students",
        detail:
          "Click 'Import from Students' → select the passing-out batch → confirm",
      },
      {
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function groupModules(modules: ModuleDoc[]): Record<string, ModuleDoc[]> {
  return modules.reduce<Record<string, ModuleDoc[]>>((acc, m) => {
...
function StepCard({
  ...
  step,
  detail,
}: { index: number; step: string; detail: string }) {
  return (
...
// ─── Module Detail Panel ──────────────────────────────────────────────────────
function ModuleDetail({ module: mod }: { module: ModuleDoc }) {
  const Icon = mod.icon;
  return (
    <div className="p-6 space-y-6" data-ocid="help.detail_panel">
      {/* Header */}
      ...
      {/* What it does */}
      <section aria-labelledby={`help-what-${mod.id}`}>
        <h3
          id={`help-what-${mod.id}`}
      ...
      {/* Steps */}
      <section aria-labelledby={`help-steps-${mod.id}`}>
        <h3
      ...
      {/* Key Features */}
      <section aria-labelledby={`help-features-${mod.id}`}>
        <h3
      ...
      >
        ...
        >
          <Lightbulb className="size-4 text-accent-foreground" /> Tips & Best
          Practices
        </h3>
        <ul className="space-y-1.5">
          {mod.tips.map((t, ti) => (
            <li
              ...
              className="flex items-start gap-2 text-sm text-muted-foreground"
            >
              <span className="shrink-0 text-accent-foreground font-bold">
                ·
              </span>
              <span>{t}</span>
            </li>
...
export default function HelpPage() {
  ...
  // When filter changes and selected module is hidden, auto-select first visible
  const visibleIds = new Set(filtered.map((m) => m.id));
  const effectiveSelected = visibleIds.has(selectedId)
    ? selectedModule
    : (filtered[0] ?? MODULES[0]);

  let listIndex = 0;

  return (
    <div className="flex h-full bg-background" data-ocid="help.page">
      ...
      >
        ...
        <div className="p-3 border-t border-border shrink-0">
          <p className="text-[10px] text-muted-foreground text-center">
            {MODULES.length} modules documented
          </p>
        </div>
      </aside>
      ...
      <main className="flex-1 min-w-0 overflow-auto bg-background">
        {effectiveSelected ? (
          <ModuleDetail module={effectiveSelected} />
        ) : (
          <div
